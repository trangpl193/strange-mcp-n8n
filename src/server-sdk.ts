...
function createMcpServer(client: N8NClient): McpServer {
  ...
  // Tool: workflow_discover (Tier 1 - Ultra-lightweight)
  server.registerTool(
    'workflow_discover',
    {
    ...
    async (args: { active?: boolean; tags?: string; name?: string; limit?: number; cursor?: string }) => {
      const result = await workflowDiscover(client, args);
      return {
  ...
  // Tool: workflow_list (Progressive Loading Tiers 1-3)
  server.registerTool(
    'workflow_list',
    {
      ...
      inputSchema: {
        ...
        mode: z.enum(['minimal', 'summary', 'detailed']).optional().describe('Detail level: "minimal" (id/name/active), "summary" (default: +tags/nodes_count/dates), "detailed" (+node_types/connections/triggers)'),
      },
    },
    async (args: { active?: boolean; tags?: string; name?: string; limit?: number; cursor?: string; mode?: 'minimal' | 'summary' | 'detailed' }) => {
  ...
  server.registerTool(
    'schema_list',
    {
      description:
        ...
        'Use summary=true for lightweight discovery (95% token reduction: ~170 tokens vs ~16k).',
      inputSchema: {
        status: z.enum(['recommended', 'deprecated', 'experimental']).optional().describe('Filter by format status (optional)'),
        summary: z.boolean().optional().describe('Return lightweight summary (recommended for discovery). Reduces response from ~16k to ~170 tokens (95% savings).'),
      },
    },
    async (args, _extra) => {
  ...
  // Tool: schema_discover (NEW - Tier 1: Progressive Loading)
  server.registerTool(
    'schema_discover',
    {
      description:
        'Discover available schemas (Tier 1: minimal discovery layer). ' +
        'Returns only node IDs and basic counts (~200 tokens for 8 schemas). ' +
        'Part of Progressive Loading Architecture to prevent session overflow. ' +
        'Use schema_get(id) to get details for specific schemas. ' +
        'Recommended: Use this instead of schema_list() for initial discovery.',
      inputSchema: {},
    },
    async (_args, _extra) => {
      const result = await schema_discover();
      return {
        content: [{ type: 'text' as const, text: JSON.stringify(result, null, 2) }],
      };
    }
